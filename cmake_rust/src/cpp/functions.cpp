...
int perf_y_ax( rust::Vec<rust::String> argv)
{
    ...
    for (size_t i = 0; i < argv.size(); i++) {
        const char* arg = argv[i].c_str();  // get const char* if you want strcmp

        if (strcmp(arg, "-N") == 0 || strcmp(arg, "-Rows") == 0) {
            N = pow(2, std::atoi(argv[++i].c_str()));
            printf("  User N is %d\n", N);
        }
        else if (strcmp(arg, "-M") == 0 || strcmp(arg, "-Columns") == 0) {
            M = pow(2, std::atoi(argv[++i].c_str()));
            printf("  User M is %d\n", M);
        }
        else if (strcmp(arg, "-S") == 0 || strcmp(arg, "-Size") == 0) {
            S = pow(2, std::atoi(argv[++i].c_str()));
            printf("  User S is %d\n", S);
        }
        else if (strcmp(arg, "-nrepeat") == 0) {
            nrepeat = std::atoi(argv[++i].c_str());
        }
