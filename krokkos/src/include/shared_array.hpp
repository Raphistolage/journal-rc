...
namespace shared_array {
    ...
    int deep_copy(SharedArrayMut_f64 &shared_arr1, const SharedArray_f64 &shared_arr2);
    ...
    SharedArray_f64 dot(const SharedArray_f64 &shared_arr1, const SharedArray_f64 &shared_arr2);
    SharedArray_f64 matrix_vector_product(const SharedArray_f64 &shared_arr1, const SharedArray_f64 &shared_arr2);
    SharedArray_f64 matrix_product(const SharedArray_f64 &shared_arr1, const SharedArray_f64 &shared_arr2);
    void mutable_matrix_product(SharedArrayMut_f64 &shared_arr1, const SharedArray_f64 &shared_arr2, const SharedArray_f64 &shared_arr3);
    ...
    void mat_add_one(SharedArrayMut_f64 shared_arr);
}
namespace shared_array {
    template <typename T = double>
    Kokkos::View<T**, Kokkos::LayoutRight, Kokkos::DefaultExecutionSpace::memory_space, Kokkos::MemoryTraits<Kokkos::Unmanaged>> view2D_from_shared(const SharedArray_f64 &shared_arr) {
    ...
    Kokkos::mdspan<const T, Kokkos::dextents<std::size_t, D>> mdspan_from_shared_impl(const SharedArray_f64 &shared_arr, std::index_sequence<Is...>) {
            const size_t* shape = shared_arr.shape;
            const T* typed_ptr = static_cast<const T*>(shared_arr.ptr);
    ...
    Kokkos::mdspan<const T, Kokkos::dextents<std::size_t, D>> mdspan_from_shared(const SharedArray_f64 &shared_arr) {
    ...
    template <typename T, int D, std::size_t... Is>
    Kokkos::mdspan<T, Kokkos::dextents<std::size_t, D>> mdspan_from_shared_mut_impl(SharedArrayMut_f64 &shared_arr, std::index_sequence<Is...>) {
            const size_t* shape = shared_arr.shape;
    ...
    template <int D, typename T = double>
    Kokkos::mdspan<T, Kokkos::dextents<std::size_t, D>> mdspan_from_shared_mut(SharedArrayMut_f64 &shared_arr) {
        if (shared_arr.rank != D) {
            throw std::runtime_error("Incompatible dimensions of cast and SharedArray");
        } else if (!shared_arr.is_mut) {
            throw std::runtime_error("Tried casting an imutable SharedArray_f64 to a mutable mdspan");
        }
        return mdspan_from_shared_mut_impl<T, D>(shared_arr, std::make_index_sequence<D>{});
    }

    template <int D, typename T>
    SharedArrayMut_f64 to_shared_mut(Kokkos::mdspan<T, Kokkos::dextents<std::size_t, D>> fromMds, bool allocated_by_cpp, MemSpace mem_space = MemSpace::HostSpace) {
    ...
    template <int D, typename T>
    SharedArray_f64 to_shared(Kokkos::mdspan<T, Kokkos::dextents<std::size_t, D>> fromMds, bool allocated_by_cpp, MemSpace mem_space = MemSpace::HostSpace) {
        int rank = fromMds.rank();
        size_t* shape = new size_t[7];
        for (int i = 0; i < rank; i++)
        {
    ...
    template <typename T = double>
    SharedArray_f64 templated_dot(const SharedArray_f64 &shared_arr1, const SharedArray_f64 &shared_arr2) {
    ...
    template <typename T = double>
    SharedArray_f64 templated_matrix_vector_product(const SharedArray_f64 &shared_arr1, const SharedArray_f64 &shared_arr2) {
    ...
    template <typename T = double>
    SharedArray_f64 templated_matrix_product(const SharedArray_f64 &shared_arr1, const SharedArray_f64 &shared_arr2) {
