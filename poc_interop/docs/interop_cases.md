...
- [x] Instantiate Rust variable (that implements toShared) 
    - cast to SharedArrayView (non-owning)
...
- [X] Instantiate Rust variable (that implements toSharedMut) 
    - cast to SharedArrayViewMut (non-owning, mutable) or ArrayView (owning, consuming the variable)
    ...
    - copy to SharedArrayViewMut (non-owning) or cast to ArrayView (owning)
...
- [x] Instantiate Rust variable (with trait ToSharedMut) 
    - cast to SharedArrayViewMut (non-owning, mutable)
...
- [x] Instantiate Kokkos::mdspan (non-owning) on C++ side with some data.
    - cast to SharedArrayView (toShared, non-owning) 
...
- [x] Instantiate Kokkos::mdspan (non-owning) on C++ side with some data.
    - cast to SharedArrayViewMut (toSharedMut, non-owning, mutable) 
    - call Rust function on it 
...
|---------------------------|-------|-----|
| Rust | impl ToShared (Mut) <br> \| <br> SharedArrayView (Mut) (Non Owning) <br> \| <br> Mdspan (non-owning) / Kokkos::View (Device, Own copy) | fn create_opaque_view(data, metadata...) <br> \| <br> OpaqueView2DF64 (Owning) <br> \| <br> Kokkos::View (Owning) |
| C++  |  | mdspan / Kokkos::View <br> \| <br> to_shared<> <br> \| <br> SharedArrayView (Mut) (Non owning) <br> \| <br> impl FromShared (Non owning) |
