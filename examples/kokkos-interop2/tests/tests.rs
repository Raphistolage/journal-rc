...
fn test_kokkos_views_equals() {
    unsafe {
        ...
        { // Scope necessaire pour dealocate les views quand finis.
            let host_view = ffi::create_rust_view(ffi::MemSpace::HostSpace, "HostView".to_string(), vec![4,3,2,1]);
            let device_view = ffi::create_rust_view(ffi::MemSpace::CudaSpace, "DeviceView".to_string(), vec![21]);
            let data1 = [42.0f64; 24];
            let data2 = [49.0f64; 22];
            
            
            // ffi::fill_view(&host_view, &mut data1);
            // ffi::fill_view(&device_view, &mut data1);
            ffi::deep_copy_data(&host_view, &mut data1);
