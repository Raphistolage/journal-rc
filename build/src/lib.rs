...
pub fn build(rust_source_file: impl AsRef<Path>) {
    ...
    let target_path = out_path
        .ancestors()
        .nth(4)
        .expect("Cannot reach the target folder.");
    ...
    if !target_path.join("krokkosbridge").exists() {
        ...
        std::fs::create_dir(target_path.join("krokkosbridge"))
            .expect("Error when creating the krokkosbridge folder.");
    }
    ...
    let modified_dst_config = dst_config
        ...
        .configure_arg(format!("-DTARGET_DIR={}", target_path.display()))
